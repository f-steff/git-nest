...
their system. The Amiga package is a separate `.lha`.
...
| Amiga (install) | AmigaShell `git-nest` wrapper, `git_nest.sh`, `lib/`, install/uninstall/filter_lines | AmigaGuide (.guide) + md, skill |

Artifacts:
- `git-nest-<v>.tar.gz` (universal, POSIX-readable)
- `git-nest-<v>.zip` (universal, Windows-readable)
- `git-nest-<v>-amiga.lha` (Amiga package, created in Alpine with `lha`)

Amiga specifics:
- The `.lha` is created inside an Alpine container (`apk add lha`); the
  classic LHarc 1.02 archiver is not packaged for Debian/Fedora (only the
  extract-only `lhasa`), so Alpine is the CI path.
- The Amiga installer (`packaging/amiga/install`) copies to
  SYS:Utilities/git-nest, adds the dir to the AmigaShell `path`, and
  persists it in S:Shell-Startup; `uninstall` removes both (with a small
  `filter_lines` companion for the S:Shell-Startup rewrite).
- AmigaGuide docs are generated by `scripts/package/generate-amigaguide.sh`
  (pure POSIX awk, no pandoc needed; handles headings, lists, code blocks,
  inline code/bold/links, strips badges).

### Amiga S (Script) bit

The Amiga `git-nest` forwarder must have the **S (Script)** protection bit
so AmigaShell runs it as a script. Unix-created LHA archives CANNOT carry
the Amiga S bit: the level-2 header's attribute byte is fixed at 0x20, the
OS type is 'U', and the Unix mode lives in an extended header where the S
bit has no POSIX equivalent (exec maps only to the Amiga E bit).

**Decision**: the Amiga `install` script sets the S+E bits itself with
`filex "$DEST/git-nest" s` and `filex ... e` after copying. Since the
installer runs on the Amiga, this is reliable and honest; the .lha marks
the file executable (E) via the Unix mode, and the installer adds S.
(Reverse-engineering the lha v1.14i header to patch os='A' + attribute
0x72 is possible but requires recomputing the level-2 header CRC and
produced extractor errors in testing; not worth the fragility.)
...
| Amiga | SYS:Utilities/git-nest or similar | AmigaShell install script adds `path`; uninstall script provided |
