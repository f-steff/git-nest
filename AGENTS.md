...
- `bin/git_nest.sh`: command, manifest, Git, branching, and finalize helpers sourced by the entrypoint.
...
- `sh tests/run-all-tests.sh`: run the full shell test suite from Git Bash or another POSIX-like shell. The runner resets `${TMPDIR:-/tmp}/git-nest-test-workspaces` at startup and leaves generated repositories in numbered folders such as `test_01_command_finalize_auto_no_pending/`.
...
Do not store provider tokens or credentials in `.gitnest`. Keep provider integration optional, and prefer `.gitnest-rc` or environment variables for local configuration. Finalized subproject entries must pin `revision=<sha>` and must not depend only on moving branch names.
