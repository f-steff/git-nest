...
$shellScript = Join-Path $scriptDir "git_nest.sh"
...
if ($IsWindows) {
    ...
    if (-not (Test-Path $shellScript)) {
        ...
        $shellScript = Get-Command git_nest.sh -ErrorAction SilentlyContinue
        if (-not $shellScript) {
            Write-Error "git_nest.sh not found next to this script or on PATH."
            exit 2
        }
    ...
    if (-not (Test-Path $shellScript)) {
        Write-Error "git_nest.sh not found at $shellScript"
        exit 2
    }
