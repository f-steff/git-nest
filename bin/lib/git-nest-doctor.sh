...
cmd_doctor() {
	...
	if gitattributes_has_guard; then
		...
		doctor_add_check "$checks" W gitattributes "missing or stale git-nest attributes guard; run git-nest tidy to refresh it"
	fi
	...
	if [ -f .gitignore ]; then
		...
		if [ -n "$stale_orphans" ]; then
			...
			doctor_add_check "$checks" W gitignore-stale "$stale_count stale nest-owned ignore entry(s); run git-nest tidy to prune them"
		else
...
survey_classify_row() {
	...
	if [ "$dcr_inside" -eq 1 ]; then
		...
		dcr_detail="detached former subproject; git-nest absorb $dcr_path_q to re-manage, or move/remove it and run git-nest tidy"
