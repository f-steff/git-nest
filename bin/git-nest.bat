...
rem still delegating behavior to the adjacent shell entrypoint, bin\git-nest.
rem If that file is not next to this wrapper, PATH is searched for git-nest.
...
set "SCRIPT=%~dp0git-nest"
if not exist "%SCRIPT%" (
    set "SCRIPT="
    for /f "delims=" %%I in ('where git-nest 2^>nul') do (
        if not defined SCRIPT set "SCRIPT=%%~fI"
    )
)

if not defined SCRIPT (
    echo Error: Could not locate git-nest next to this wrapper or on PATH.
    exit /b 2
)
