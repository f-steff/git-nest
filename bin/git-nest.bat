...
rem still delegating behavior to the adjacent implementation file, bin\git_nest.sh
...
set "SCRIPT=%~dp0git_nest.sh"
if not exist "%SCRIPT%" (
    set "SCRIPT="
    for /f "delims=" %%I in ('where git_nest.sh 2^>nul') do (
        if not defined SCRIPT set "SCRIPT=%%~fI"
    )
)

if not defined SCRIPT (
    echo Error: Could not locate git_nest.sh next to this wrapper or on PATH.
    exit /b 2
)
