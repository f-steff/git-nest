...
- `init [--rc] [--sure]`: creates a new nest. Existing nest roots are reported as already initialized and are not repaired. Inside a managed subproject, plain `init` refuses and requires `--sure`. Even with `--sure`, `init` (and `absorb-all`'s auto-init) always refuses when the new nest's own subtree would contain a path already registered as a subproject by an ancestor nest, naming the conflicting ancestor nest, the swallowed path, and the exact manual recipe to resolve it (`detach` the subproject from the ancestor, retry `init` here, then `absorb` it back into the new nest); there is no override for this refusal. `init` also creates `NEST_README.md` in the nest root if it does not exist: a short pointer explaining that the repository is a git-nest workspace and how to materialize it with `git-nest restore`. The file is created only (never overwritten), so the maintainer can edit, commit, ignore, or delete it freely.
- `tidy [--rc]`: refreshes managed support files for an existing nest, reconciles the managed `.gitignore` block, and prunes stale nest-owned ignore entries (reporting each pruned entry). It does not recreate a deleted `NEST_README.md`; that file is created by `init` only.
...
- `interactive` (alias `ii`): runs a line-based guided menu over the command surface. It never enters raw terminal mode or emits cursor control; menus are numbered lines and input is plain lines, so it works from any host (Git Bash, cmd.exe, PowerShell) and from piped stdin. The menu is context-filtered: a folder with no Git repository up the tree offers the real `git init`; a Git repository without `.gitnest` offers `git-nest init`; a nest offers the full grouped surface. Context is re-detected after every action. Every action prints `cwd>command ...` and runs in a sub-shell with inherited stdio; a nonzero action exit is shown but does not end the session. Input tokens: a number runs the entry, `b` or an empty line goes back (at the top-level menu it reports that there is no previous menu), `q` exits with code 0, and EOF (or an exhausted scripted input list) also exits gracefully. Arguments for text-taking commands are read on the next line and appended verbatim. `change directory` and `move` share a folder browser that navigates one level at a time: `0` goes to the parent (hidden at the browser cap or the filesystem root), numbers enter folders, and on the first screen only the three relevant start points (nest root, the folder the session started in, the session's current folder, deduplicated by path) are offered as the first numbered entries. In cd mode the navigation persists and updates the session cwd; in select mode the session cwd is restored on exit and `s`/`n` return a selection (`s` = the current folder, `n` = the folder plus a prompted name), with the browser capped at the nest root so the result is always nest-relative (selecting the cap itself with `s` is refused). `move` picks the source subproject, then browses for the destination in select mode. `jump nest` (all contexts) lists nested nests -- managed subprojects carrying their own manifest plus unmanaged nest roots from `survey --porcelain` -- and the session's visited-nest history (deduplicated, most recent first, capped at ten, current nest excluded), and moves the session cwd to the picked one, printing `Now in nest: <path>`. The "Nest contents" group folds the membership verbs into two flows: `bring in (absorb)` runs `survey`, lists the detected unmanaged targets from `survey --porcelain` with their kinds, and absorbs a picked one through the right form (plain `absorb` for submodules, nested repos, nest roots, and detached former subprojects; `absorb --subrepo` for git-subrepos), with `a` for bulk absorb-all (confirmed) and `m` for a manual path (folders without `.git` are prompted for a remote URL); `take out (inline/detach/remove)` runs `tree --all`, shows a picker of managed subprojects, and lets the user choose inline, detach, or remove, each confirmed with `[y/N]`. Internal switches `--ii-test <token>...` (scripted input lines, echoed as consumed) and `--ii-skip <n>` (drop the first n tokens without executing them) exist for test automation only and are not exposed in completion.
