...
- `tree [--all] [--recursive]`: displays an ASCII-art tree of the current nest, grouping managed subprojects by shared path prefixes. `--all` also shows `survey`'s own detected-but-unmanaged findings, each marked with its code, without touching state. `--recursive` also descends into nested nests, rendering their own subprojects nested under that branch. Uses ASCII connectors only (`|--`, `` `-- ``, `|`); no Unicode box-drawing characters.
