...
- `tree [--all] [--recursive] [--porcelain | --json | --json-pretty]`: displays an ASCII-art tree of the current nest, grouping managed subprojects by shared path prefixes. `--all` also shows `survey`'s own detected-but-unmanaged findings, each marked with its code, without touching state. `--recursive` also descends into nested nests, rendering their own subprojects nested under that branch. Every branch uses a single `+--` connector (never a different glyph for the last child of a level) and every entry gets a trailing `/`; continuation columns use `|` or blank spaces per level; no Unicode box-drawing characters. `--porcelain` prints the same 7-column fixed-field rows as the other machine-readable commands; `--json`/`--json-pretty` wrap them in the shared envelope.
