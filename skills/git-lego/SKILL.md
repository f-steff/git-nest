...
Write-side commands preserve project boundaries. From a parent project, do not run path-changing commands against paths inside a nested project; run from the nested project root instead. Use `snapshot --recursive` only when the user wants recursive local manifest refresh. Keep current-project scoped commands such as `diff`, `foreach-*`, and `export` rooted where the user asked you to run them.
...
`snapshot` updates local manifest state without pushing. `upload` pushes changed subproject branches and records pending subproject state in `.gitlego`. `upload --finalize` pushes changed subproject branches and records the pushed commits directly as finalized revisions when the user does not want a separate review/finalize step. `no-pending` fails while pending subprojects remain.
...
Only extract or absorb subprojects when the user explicitly asks:
