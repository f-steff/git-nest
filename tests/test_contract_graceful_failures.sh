...
set -eu
. "$(dirname "$0")/helper.sh"
test_begin contract_graceful_failures

test_step "Exercise contract graceful failures" "This test verifies the documented contract graceful failures behavior and fails if command output or repository state differs from the expected result."

work=$(test_workspace contract_graceful_failures)
...
if "$GIT_NEST" finalize libs/foo --revision >finalize_missing_value.out 2>finalize_missing_value.err; then
...
if "$GIT_NEST" finalize libs/foo --revision HEAD --tag v1 >finalize_combo.out 2>finalize_combo.err; then
...
if "$GIT_NEST" finalize libs/missing --revision HEAD >finalize_missing_subproject.out 2>finalize_missing_subproject.err; then
...
if "$GIT_NEST" upload >detached_upload.out 2>detached_upload.err; then
...
if "$GIT_NEST" upload >no_origin_upload.out 2>no_origin_upload.err; then
...
cp .gitnest manifest.before
if "$GIT_NEST" finalize libs/foo --revision does-not-exist >bad_revision.out 2>bad_revision.err; then
...
cmp .gitnest manifest.before >/dev/null

# A missing target branch should fail before finalization mutates the manifest.
sed 's/^target_branch=.*/target_branch=missing-target/' .gitnest >.gitnest.tmp
mv .gitnest.tmp .gitnest
cp .gitnest manifest.bad-target
if "$GIT_NEST" finalize libs/foo --use-target-head >bad_target.out 2>bad_target.err; then
...
cmp .gitnest manifest.bad-target >/dev/null

# A malformed subproject section without repo= should fail instead of writing empty state.
sed '/^repo=/d' .gitnest >.gitnest.tmp
mv .gitnest.tmp .gitnest
if "$GIT_NEST" snapshot >missing_repo.out 2>missing_repo.err; then
...
describe_result "The contract graceful failures behavior matched the expected command output and repository state."
