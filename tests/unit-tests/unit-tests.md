...
Unlike the integration tests under `tests/integration-tests/`, unit tests do **not** create real Git repositories, remotes, or bare clones. They use a mock Git shim that intercepts all `git` calls and returns canned responses.
...
|-- unit-tests.ini     # Deliberately untested functions (coverage map)
|-- unit-tests.md      # This file
|-- unit-test_NNNN_<module>_<name>.sh
```
...
Each test file lists the functions it covers in `# Coverage:` headers. The runner collects all coverage headers, compares them against all function definitions in `bin/lib/*.sh`, and reports (this is the default; pass `--no-coverage` to skip):

```
--- Coverage Report ---
Covered: 86/313 = 27.5%
Deliberately untested: 227
Unclassified: 0
```

The exact numbers drift as code changes; the report is authoritative. Functions listed in `unit-tests.ini` under the `[untested]` section are considered
...
```sh
sh tests/unit-tests/run-all-tests.sh              # Run all unit tests
sh tests/unit-tests/run-all-tests.sh list         # List all tests as ID + description
sh tests/unit-tests/run-all-tests.sh only 1000,1010 # Run only given IDs
sh tests/unit-tests/run-all-tests.sh except 1020   # Run all except given ID
sh tests/unit-tests/run-all-tests.sh help          # Print help
```
...
The full suite runs the unit tests automatically through
`tests/integration-tests/test_0000_unit_tests.sh`, which iterates every
`unit-test_*.sh` here and reports PASS/FAIL line by line:

```sh
sh tests/run-all-tests.sh
```

To run only the unit suite, use the standalone runner above.

## Test ID allocation

Unit tests use block 1000--1999. Within the block, sub-ranges by source module
(the exact files use `unit-test_<NNNN>_<module>_<name>.sh`):

| Range | Category | What it covers |
|-------|----------|---------------|
| 1000--1099 | unit_manifest | Path safety, normalize, clone mode |
| 1200--1299 | unit_helper | `shell_quote`, `json_escape`, `config_get` |
| 1300--1399 | unit_clone | Clone mode functions |
| 1400--1499 | unit_commands/unit_doctor/unit_manifest | Command, doctor, and manifest helpers |
| 1500--1999 | unit_* | Extended coverage (manifest, conversion, doctor) |
