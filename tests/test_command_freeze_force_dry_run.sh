...
"$GIT_LEGO" add "$url" freeze/one >/dev/null
...
run_capture "dry-run described the freeze target" freeze_dry.out freeze_dry.err -- "$GIT_LEGO" freeze --only freeze/one --dry-run --force
...
run_fail "unsafe freeze refused" any -- sh -c '"$1" freeze --only freeze/one >freeze_refuse.out 2>freeze_refuse.err' sh "$GIT_LEGO"
...
run_capture "forced freeze pinned current HEAD" freeze_force.out freeze_force.err -- "$GIT_LEGO" freeze --only freeze/one --force
