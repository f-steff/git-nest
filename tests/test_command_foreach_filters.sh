...
run_capture "dirty subproject listed in porcelain output" foreach_modified.out foreach_modified.err -- "$GIT_LEGO" foreach-modified --porcelain
...
run_capture "clean subprojects listed in porcelain output" foreach_clean.out foreach_clean.err -- "$GIT_LEGO" foreach-clean --porcelain
...
run_capture "dirty subproject listed in JSON output" foreach_modified.json foreach_modified_json.err -- "$GIT_LEGO" foreach-modified --json
...
run_ok "foreach-modified ran only in the dirty subproject" -- "$GIT_LEGO" foreach-modified -- sh -c 'printf "%s\n" "$GIT_LEGO_SUBPROJECT_PATH" >>"$GIT_LEGO_ROOT/modified_command.out"'
