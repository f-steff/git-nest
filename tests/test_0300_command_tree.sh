...
describe_result "tree grouped managed subprojects by shared path prefix, showed root [N] with [Nest Root], managed [M] with [Managed] and URLs, --all added survey findings with [R]/[Unmanaged Repo], --recursive nested a nested nest's subprojects, --plain omitted URL and type columns, --porcelain and --json emitted the shared schema with typelabel in state and url in detail."
