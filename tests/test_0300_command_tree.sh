...
test_step "Exercise tree grouping, --all, --recursive, and --json" "tree must group by shared path prefixes with ASCII-only connectors, --all must add survey's own findings distinctly marked, --recursive must nest a nested nest's own subprojects under its branch, and --json must emit the shared row schema."
...
# --- Plain tree groups by shared prefix with ASCII-only connectors ---
test_step "Plain tree groups managed subprojects by shared path prefix" "libs/foo and libs/bar must render as one libs branch with two children; connectors must be pure ASCII."
...
describe_result "tree grouped managed subprojects by shared path prefix with ASCII-only connectors, --all added survey's own unmanaged findings distinctly marked, --recursive nested a nested nest's own subprojects under its branch, and --json emitted the shared row schema."
