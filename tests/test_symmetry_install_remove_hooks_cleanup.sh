...
run_ok "managed hooks installed while recording current state" -- "$GIT_LEGO" start . --hooks
assert_managed_hooks "."
...
run_ok "newly added subproject inherited hooks" -- "$GIT_LEGO" add "$remote_three" libs/three
...
run_ok "install-hooks remained idempotent" -- "$GIT_LEGO" install-hooks
...
run_ok "finalize --cleanup removed local pending branch" -- "$GIT_LEGO" finalize libs/one --revision "$merge_sha" --cleanup
...
printf 'keep\n' >libs/one/keep.txt
run_ok "cleanup-branches left unrelated files alone" -- "$GIT_LEGO" cleanup-branches
...
run_ok "managed hooks removed" -- "$GIT_LEGO" remove-hooks
assert_no_managed_hooks "."
...
run_fail "unmanaged hook prevented partial installation" any -- sh -c '"$1" install-hooks >hooks.out 2>hooks.err' sh "$GIT_LEGO"
