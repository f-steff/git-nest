...
rem fallback below. The actual test logic stays in run-all.sh.
...
set "SCRIPT=%~dp0run-all.sh"
if not exist "%SCRIPT%" (
    echo Error: Could not locate tests\run-all.sh next to this wrapper.
    exit /b 2
)
...
# all suite behavior in run-all.sh so Windows and POSIX entrypoints stay aligned.
