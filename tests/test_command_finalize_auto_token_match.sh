...
run_ok "project branch started for AB-1" -- "$GIT_NEST" start AB-1-token-test
printf 'ab1\n' >>libs/foo/file.txt
git -C libs/foo add file.txt
git -C libs/foo commit -m "subproject change" >/dev/null
run_ok "subproject work uploaded and recorded as pending" -- "$GIT_NEST" upload
...
run_ok "pending state finalized through token-bound auto-detection" -- "$GIT_NEST" finalize libs/foo
