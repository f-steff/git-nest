...
run_fail "dirty workspace canceled before branch switching" any -- sh -c '"$1" start --cancel-dirty XX-888-dirty >cancel.out 2>cancel.err' sh "$GIT_LEGO"
...
run_ok "dirty work stashed and project branch started" -- "$GIT_LEGO" start --stash-dirty XX-888-dirty
...
run_fail "untracked file blocked discard mode" any -- sh -c '"$1" start --discard-dirty XX-999-discard >discard.out 2>discard.err' sh "$GIT_LEGO"
