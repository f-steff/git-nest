...
set -eu
. "$(dirname "$0")/helper.sh"
test_begin update_command

root=$(test_workspace update_command)
...
"$GIT_STACK" update libs/foo --target-head >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$second"
assert_file_contains .stack "revision=$second"
assert_file_not_contains .stack "tag="
...
"$GIT_STACK" update libs/foo --tag v2.0.0 >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$second"
assert_file_contains .stack "tag=v2.0.0"
assert_file_contains .stack "revision=$second"
...
"$GIT_STACK" update libs/foo --revision "$third" >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$third"
assert_file_contains .stack "revision=$third"
assert_file_not_contains .stack "tag=v2.0.0"

# --no-fetch should use only refs already present in the module checkout.
printf 'fourth\n' >>"$seed/file.txt"
git -C "$seed" add file.txt
...
"$GIT_STACK" update libs/foo --remote --no-fetch >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$third"
assert_file_contains .stack "revision=$third"

# --remote is an alias for --target-head and should fetch before resolving.
"$GIT_STACK" update libs/foo --remote >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$fourth"
assert_file_contains .stack "revision=$fourth"
...
"$GIT_STACK" update libs/foo --set-branch release/1 --remote >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$release"
assert_file_contains .stack "target_branch=release/1"
assert_file_contains .stack "revision=$release"

if "$GIT_STACK" update libs/foo --branch release/1 --tag v2.0.0 >branch_tag.out 2>branch_tag.err; then
...
if "$GIT_STACK" update libs/foo --tag v3.0.0 --no-fetch >no_fetch_tag.out 2>no_fetch_tag.err; then
    echo "update --no-fetch should fail for an unfetched tag" >&2
    exit 1
fi
assert_file_contains no_fetch_tag.err "does not resolve"

# Dirty modules must be cleaned before update so checkout and manifest stay aligned.
printf 'dirty\n' >>libs/foo/file.txt
if "$GIT_STACK" update libs/foo --target-head >dirty.out 2>dirty.err; then
    echo "update should fail on dirty stack modules" >&2
    exit 1
fi
assert_file_contains dirty.err "uncommitted changes"
git -C libs/foo checkout -- file.txt

# Pending modules must be finalized or cleared before update rewrites version state.
git -C libs/foo checkout -b UPD-100-module >/dev/null
printf 'pending\n' >>libs/foo/file.txt
git -C libs/foo add file.txt
git -C libs/foo commit -m "UPD-100 pending" >/dev/null
"$GIT_STACK" upload >/dev/null
if "$GIT_STACK" update libs/foo --target-head >pending.out 2>pending.err; then
    echo "update should fail on pending stack modules" >&2
    exit 1
fi
