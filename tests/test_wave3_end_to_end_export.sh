...
set -eu
. "$(dirname "$0")/helper.sh"
test_begin wave3_end_to_end_export

root=$(test_workspace wave3_end_to_end_export)
...
clone="$root/clone"
"$GIT_LEGO" clone "$outer_remote" "$clone" >/dev/null
...
"$GIT_LEGO" foreach-modified -- sh -c '
    git add file.txt
    git commit -m "end to end export change" >/dev/null
'

"$GIT_LEGO" snapshot --no-fetch >/dev/null
"$GIT_LEGO" freeze --force >/dev/null
"$GIT_LEGO" export --output "$root/e2e-a.tar.gz" --deterministic >/dev/null
"$GIT_LEGO" export --output "$root/e2e-b.tar.gz" --deterministic >/dev/null
