#!/bin/sh
# Test: update moves a subproject to a remote head, revision, or tag
...
"$GIT_NEST" update libs/foo --target-head >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$second"
assert_file_contains .gitnest "revision=$second"
assert_file_not_contains .gitnest "tag="
...
"$GIT_NEST" update libs/foo --tag v2.0.0 >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$second"
assert_file_contains .gitnest "tag=v2.0.0"
assert_file_contains .gitnest "revision=$second"
...
"$GIT_NEST" update libs/foo --revision "$third" >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$third"
assert_file_contains .gitnest "revision=$third"
assert_file_not_contains .gitnest "tag=v2.0.0"
...
"$GIT_NEST" update libs/foo --remote --no-fetch >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$third"
assert_file_contains .gitnest "revision=$third"

# --remote is an alias for --target-head and should fetch before resolving.
"$GIT_NEST" update libs/foo --remote >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$fourth"
assert_file_contains .gitnest "revision=$fourth"
...
"$GIT_NEST" update libs/foo --set-branch release/1 --remote >/dev/null
test "$(git -C libs/foo rev-parse HEAD)" = "$release"
assert_file_contains .gitnest "target_branch=release/1"
assert_file_contains .gitnest "revision=$release"

if "$GIT_NEST" update libs/foo --branch release/1 --tag v2.0.0 >branch_tag.out 2>branch_tag.err; then
...
if "$GIT_NEST" update libs/foo --tag v3.0.0 --no-fetch >no_fetch_tag.out 2>no_fetch_tag.err; then
...
printf 'dirty\n' >>libs/foo/file.txt
if "$GIT_NEST" update libs/foo --target-head >dirty.out 2>dirty.err; then
