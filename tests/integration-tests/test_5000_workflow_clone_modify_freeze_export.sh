#!/bin/sh
# Test: clone, modify, freeze, and export produce deterministic archives
...
cd "$clone"
git -C libs/foo config user.name "git-nest test"
git -C libs/foo config user.email "git-nest@example.invalid"
printf 'end to end\n' >>libs/foo/file.txt
...
run_ok "dirty subproject committed through foreach-modified" -- "$GIT_NEST" foreach-modified -- sh -c '
    git add file.txt
...
run_ok "local pending state refreshed without fetching" -- "$GIT_NEST" snapshot --no-fetch
run_ok "current checkout pinned into the manifest" -- "$GIT_NEST" freeze --force
run_ok "first deterministic export created" -- "$GIT_NEST" export --output "$root/e2e-a.tar.gz" --deterministic
run_ok "second deterministic export created" -- "$GIT_NEST" export --output "$root/e2e-b.tar.gz" --deterministic
