#!/bin/sh
# Test: recursive commands operate across nested nests

set -eu
. "$(dirname "$0")/helper.sh"
test_begin workflow_nested_projects

test_step "Exercise workflow nested projects" "This test verifies the documented workflow nested projects behavior and fails if command output or repository state differs from the expected result."
...
EOF
    ...
    git -C "$work" add root.txt .gitnest .gitignore
...
root=$(test_workspace workflow_nested_projects)
...
"$GIT_NEST" verify >verify.out 2>verify.err
...
cd firmware
git nest status >"$root/inner_status.out"
...
"$GIT_NEST" status --porcelain >"$root/status_porcelain.out"
test ! -s "$root/status_porcelain.out"
"$GIT_NEST" status --recursive --porcelain >"$root/status_recursive_porcelain.out"
assert_file_contains "$root/status_recursive_porcelain.out" "D${tab}firmware/drivers/io${tab}dirty${tab}-${tab}-${tab}-${tab} M root.txt"
"$GIT_NEST" status --recursive --json >"$root/status_recursive.json"
...
"$GIT_NEST" verify --recursive >verify_recursive.out
...
"$GIT_NEST" verify --recursive --json >verify_recursive.json
...
"$GIT_NEST" outdated --porcelain >outdated_porcelain.out
test ! -s outdated_porcelain.out
if "$GIT_NEST" outdated --recursive --porcelain >outdated_recursive_porcelain.out; then
...
if "$GIT_NEST" outdated --recursive --json >outdated_recursive.json; then
...
"$GIT_NEST" log --recursive --max-count 20 >log_recursive.out
...
describe_result "The workflow nested projects behavior matched the expected command output and repository state."
