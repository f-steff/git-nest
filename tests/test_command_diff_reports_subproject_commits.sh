...
test_step "Create a subproject commit ahead of the manifest revision" "diff should report subproject commits not represented by .gitlego."
printf 'local feature\n' >>libs/one/file.txt
git -C libs/one add file.txt
git -C libs/one commit -m "local feature for diff" >/dev/null

run_fail "human diff reports the local subproject commit" 1 -- sh -c '"$1" diff >diff.out 2>diff.err' sh "$GIT_LEGO"
assert_file_contains diff.out "libs/one:"
assert_file_contains diff.out "local feature for diff"
run_fail "diff --stat reports changed file details" 1 -- sh -c '"$1" diff --stat >diff_stat.out 2>diff_stat.err' sh "$GIT_LEGO"
assert_file_contains diff_stat.out "local feature for diff"
assert_file_contains diff_stat.out "file.txt"
run_fail "diff --json reports machine-readable commit rows" 1 -- sh -c '"$1" diff --json >diff.json 2>diff_json.err' sh "$GIT_LEGO"
...
run_capture "current manifest has no subproject differences" diff_clean.out diff_clean.err -- "$GIT_LEGO" diff
assert_file_contains diff_clean.out "No subproject commit differences"
run_fail "diff --since reports the earlier manifest difference" 1 -- sh -c '"$1" diff --since HEAD~1 >diff_since.out 2>diff_since.err' sh "$GIT_LEGO"
assert_file_contains diff_since.out "local feature for diff"
run_fail "diff --since rejects missing refs" any -- sh -c '"$1" diff --since does-not-exist >diff_missing.out 2>diff_missing.err' sh "$GIT_LEGO"
